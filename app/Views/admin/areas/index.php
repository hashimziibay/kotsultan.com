...
<div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 mb-8 shadow-xs flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
    <div>
        <h2 class="text-xl font-extrabold text-slate-900 dark:text-white"><?= lang('App.admin_areas_title') ?></h2>
        <p class="text-xs text-slate-500 mt-1"><?= lang('App.admin_areas_sub') ?> (<?= lang('App.admin_total') ?>: <?= count($areas) ?>)</p>
    </div>
    <a href="<?= base_url('admin/areas/create') ?>" class="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-bold transition-all shadow-xs flex items-center gap-2">
        <i data-lucide="plus-circle" class="w-4 h-4"></i>
        <span><?= lang('App.admin_add_area') ?></span>
    </a>
</div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-xs text-left rtl:text-right">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60 font-medium">
                ...
                <?php $cnt = $countsMap[$a['id']] ?? 0; ?>
                <tr class="hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition-colors">
                    <td class="px-4 py-3 font-mono text-slate-400">#<?= $a['id'] ?></td>
                    <td class="px-4 py-3 font-bold text-slate-900 dark:text-white"><?= esc($a['name_en']) ?></td>
                    <td class="px-4 py-3 font-urdu text-sm" dir="rtl"><?= esc($a['name_ur']) ?></td>
                    <td class="px-4 py-3">
                    ...
                    <td class="px-4 py-3 text-center">
                        <span class="px-2.5 py-1 rounded-full text-[10px] font-extrabold uppercase <?= ($a['status'] ?? 'active') === 'active' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300' : 'bg-slate-200 text-slate-700' ?>">
                            <?= esc($a['status'] ?? 'active') ?>
                        </span>
                    </td>
                    <td class="px-4 py-3 text-right rtl:text-left">
                        <div class="flex items-center justify-end gap-1.5">
                            <a href="<?= base_url('admin/areas/edit/' . $a['id']) ?>" class="p-1.5 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-emerald-500" title="<?= lang('App.admin_edit') ?>">
                                <i data-lucide="edit-3" class="w-4 h-4"></i>
                            </a>
                            <form action="<?= base_url('admin/areas/delete/' . $a['id']) ?>" method="POST" onsubmit="return confirm('<?= lang('App.admin_confirm_delete_area') ?>');" class="inline">
                                <?= csrf_field() ?>
